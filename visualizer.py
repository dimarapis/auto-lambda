...
def semantic_colorize(semantic):
    # Define the dictionary that maps class labels to RGB colors
    color_dict = {
        -1: (255,255,255),
    ...
    #if semantic.shape[0] < 100:
    #print(semantic.shape)
    #import torchvision
    #print(torchvision.transforms.functional.get_image_num_channels(semantic))
    
    semantic = np.transpose(semantic, (1,2,0))

    img = Image.new('RGB', (semantic.shape[1], semantic.shape[0]), color=(0, 0, 0))
    #print(semantic.shape)
    # Loop over each pixel in the semantic output array
    if semantic.shape[2] == 1:
        for i in range(semantic.shape[0]):
            for j in range(semantic.shape[1]):
                # Get the class label of the current pixel
                class_label = semantic[i, j, :1][0]
                #print(class_label)
                if class_label >= 0:
                    highest_label = class_label                # Set the corresponding RGB value in the blank image
                img.putpixel((j, i), color_dict[class_label])
        ...
        for i in range(semantic.shape[0]):
            for j in range(semantic.shape[1]):
                # Get the class label of the current pixel
                class_label = semantic[i, j, :].argmax()
                if class_label >= 0:
                    highest_label = class_label
                #print(class_label)
                
                # Set the corresponding RGB value in the blank image
                img.putpixel((j, i), color_dict[class_label])
                
        #print(torchvision.transforms.functional.get_image_num_channels(img))
...
def normal_colorize(image):
    image = (image - np.min(image)) / ((np.max(image) - np.min(image))) * 255
    #print(image.shape)
    return(np.transpose(image,(1,2,0))).astype('uint8')
